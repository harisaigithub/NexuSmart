...
from school import models as TMODEL
from school import models as SMODEL
from school import forms as TFORM
from school import forms as SFORM
...
def teacher_notes(request):
    return  render(request,'school/teacher_notes.html')
...
def teacher_upload_notes(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('teacher-dashboard')  # Redirect to the notes management page
    else:
        form = NoteForm()
    return render(request, 'school/teacher_upload_notes.html', {'form': form})

def teacher_view_notes(request):
    notes = Note.objects.all()
    return render(request, 'school/teacher_view_notes.html', {'notes': notes})
...
def delete_note(request, note_id):
    ...
    return redirect('school/teacher_view_notes.html')
...
def student_view_notes(request):
    notes = Note.objects.all()
    return render(request, 'school/student_view_notes.html', {'notes': notes})
